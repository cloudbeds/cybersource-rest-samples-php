...
function AuthorizationWithInstrumentIdentifierTokenCreation()
{
    ...
    try {
        ...
        /** @var \CyberSource\Model\PtsV2PaymentsPost201Response $paymentResponse */
        $paymentResponse = $apiResponse[0];
        var_dump($paymentResponse->getId());
        $tokenInformation = $paymentResponse->getTokenInformation();
        var_dump($tokenInformation->getCustomer()->getId());
        var_dump($tokenInformation->getPaymentInstrument()->getId());
        var_dump($tokenInformation->getInstrumentIdentifier()->getId());
        var_dump($apiResponse);
        CapturePayment($paymentResponse->getId(), 50);
        RefundCapture($paymentResponse->getId(),50);
        PurchaseWithCustomerTokenId($tokenInformation->getCustomer()->getId());

        WriteLogAudit($paymentResponse);
        return $apiResponse;
...
function RefundCapture(string $gatewayAuthorisation, float $amount)
{
    $merchantReferenceCode = Uuid::uuid4()->toString();
    $clientReferenceInformationArr = [
        "code" => $merchantReferenceCode
    ...
    try {
        $apiResponse = $api_instance->refundCapture($requestObj, $gatewayAuthorisation);
        print_r(PHP_EOL);
        print_r($apiResponse);
...
function CapturePayment(string $id, float $amount)
{
    $merchantReferenceCode = Uuid::uuid4()->toString();
    $clientReferenceInformationArr = [
        "code" => $merchantReferenceCode
